...
class TestIngestCorpus:
    def test_ingests_crawled_pages_into_sources_and_chunks(self, db_session, test_config):
        ...
        with (
            patch("app.ingestion.ingest_service.load_any_corpus_config", return_value=test_config),
    ...
    def test_fetches_and_ingests_pdf_file_links(self, db_session, test_config):
        ...
        with (
            ...
            patch("app.ingestion.ingest_service.embeddings_available", return_value=False),
            patch(
                "app.ingestion.ingest_service._fetch_file_text",
                return_value="PDF body text here.",
            ),
        ):
            result = ingest_corpus(TEST_CORPUS, db_session)
    ...
    def test_fetches_and_ingests_docx_file_links(self, db_session, test_config):
        fake_result = CrawlResult(
            pages=[],
            file_links=[
                {
                    "file_url": "https://x/handbook.docx",
                    "source_page": "https://x/tutorial",
                    "file_type": "docx",
                }
            ],
        ...
        with (
            ...
            patch(
                "app.ingestion.ingest_service._fetch_file_text",
                return_value="DOCX body text here.",
            ),
        ):
            result = ingest_corpus(TEST_CORPUS, db_session)
        ...
        stored = (
            db_session.query(Source)
            .filter(Source.corpus == TEST_CORPUS, Source.url == "https://x/handbook.docx")
            .one()
        )
        assert stored.source_type == "file"
    ...
    def test_skips_unsupported_file_types(self, db_session, test_config):
        fake_result = CrawlResult(
            pages=[],
            file_links=[
                {
                    "file_url": "https://x/notes.pptx",
