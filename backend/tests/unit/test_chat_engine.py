"""Agent orchestration tests — search() and the LLM provider are mocked;
query correctness is covered by the retrieval integration tests, and
provider HTTP behavior by test_llm_providers.py."""

from unittest.mock import MagicMock, patch

from app.agent.chat_engine import (
    NO_RESULTS_MESSAGE,
    build_context,
    extract_answer_from_top_chunk,
    format_sources,
    get_reply,
    match_navigation_shortcut,
    prepare_reply_context,
    stream_reply_events,
)
from app.config import CorpusConfig, NavigationShortcut
from app.llm.prompt_builder import HistoryTurn
from app.retrieval.result import SearchResult

CONFIG = CorpusConfig(name="test", persona="You are a test assistant.")

REGISTER_SHORTCUT = NavigationShortcut(
    title="Registering for a course",
    keywords=["register for a course", "course registration"],
    steps=["Log into the portal.", "Search for the course.", "Click Register."],
    url="https://example.com/register",
)


def _result(text="chunk text", url="https://x/a", title="A", category="General", score=0.9):
    return SearchResult(
        chunk_text=text, url=url, title=title, category=category, source_type="page", score=score
    )


class TestBuildContext:
    def test_joins_chunk_texts_with_separator(self):
        results = [_result("first"), _result("second")]
        assert build_context(results) == "first\n\n---\n\nsecond"

    def test_stops_before_exceeding_max_chars(self):
        results = [_result("a" * 10), _result("b" * 10)]
        context = build_context(results, max_chars=15)
        assert context == "a" * 10

    def test_empty_results_gives_empty_string(self):
        assert build_context([]) == ""


class TestExtractAnswerFromTopChunk:
    def test_returns_cleaned_top_chunk_when_short(self):
        results = [_result("This is a short answer that fits easily within limits.")]
        answer = extract_answer_from_top_chunk(results, max_chars=200)
        assert answer == "This is a short answer that fits easily within limits."

    def test_truncates_long_chunk_at_sentence_boundary(self):
        long_text = ("Sentence one is here. " * 40).strip()
        results = [_result(long_text)]
        answer = extract_answer_from_top_chunk(results, max_chars=100)
        assert answer.endswith(".")
        assert len(answer) <= 100

    def test_no_results_returns_no_results_message(self):
        assert extract_answer_from_top_chunk([]) == NO_RESULTS_MESSAGE


class TestFormatSources:
    def test_dedupes_by_url(self):
        results = [_result(url="https://x/a"), _result(url="https://x/a"), _result(url="https://x/b")]
        sources = format_sources(results)
        assert [s["url"] for s in sources] == ["https://x/a", "https://x/b"]

    def test_includes_title_and_category(self):
        results = [_result(url="https://x/a", title="Fees Page", category="Fees")]
        sources = format_sources(results)
        assert sources[0] == {"url": "https://x/a", "title": "Fees Page", "category": "Fees"}


class TestMatchNavigationShortcut:
    def test_matches_on_keyword_present_in_query(self):
        config = CorpusConfig(name="t", persona="p", navigation_shortcuts=[REGISTER_SHORTCUT])
        match = match_navigation_shortcut("How do I register for a course?", config)
        assert match is REGISTER_SHORTCUT

    def test_matching_is_case_insensitive(self):
        config = CorpusConfig(name="t", persona="p", navigation_shortcuts=[REGISTER_SHORTCUT])
        match = match_navigation_shortcut("REGISTER FOR A COURSE please", config)
        assert match is REGISTER_SHORTCUT

    def test_returns_none_when_no_keyword_matches(self):
        config = CorpusConfig(name="t", persona="p", navigation_shortcuts=[REGISTER_SHORTCUT])
        assert match_navigation_shortcut("What are the tuition fees?", config) is None

    def test_returns_none_when_corpus_has_no_shortcuts(self):
        assert match_navigation_shortcut("register for a course", CONFIG) is None

    def test_returns_first_matching_shortcut_in_config_order(self):
        other = NavigationShortcut(title="Other", keywords=["register"], steps=["Step."])
        config = CorpusConfig(
            name="t", persona="p", navigation_shortcuts=[REGISTER_SHORTCUT, other]
        )
        assert match_navigation_shortcut("register for a course", config) is REGISTER_SHORTCUT


class TestGetReply:
    def test_returns_shortcut_reply_without_calling_search_or_llm(self):
        config = CorpusConfig(name="t", persona="p", navigation_shortcuts=[REGISTER_SHORTCUT])
        with (
            patch("app.agent.chat_engine.search") as mock_search,
            patch("app.agent.chat_engine.get_llm_provider") as mock_factory,
        ):
            reply = get_reply(session=object(), corpus_config=config, query="register for a course")

        mock_search.assert_not_called()
        mock_factory.assert_not_called()
        assert reply["text"] == (
            "Registering for a course\n\n"
            "1. Log into the portal.\n"
            "2. Search for the course.\n"
            "3. Click Register."
        )
        assert reply["sources"] == [
            {
                "url": "https://example.com/register",
                "title": "Registering for a course",
                "category": "Navigation",
            }
        ]

    def test_shortcut_without_url_has_no_sources(self):
        shortcut = NavigationShortcut(title="No URL", keywords=["no-url-shortcut"], steps=["Step."])
        config = CorpusConfig(name="t", persona="p", navigation_shortcuts=[shortcut])
        reply = get_reply(session=object(), corpus_config=config, query="no-url-shortcut please")
        assert reply["sources"] == []


    def test_no_results_short_circuits_with_no_results_message(self):
        with patch("app.agent.chat_engine.search", return_value=[]):
            reply = get_reply(session=object(), corpus_config=CONFIG, query="anything")
        assert reply == {"text": NO_RESULTS_MESSAGE, "sources": []}

    def test_uses_llm_answer_and_sources_on_success(self):
        results = [_result(text="Fees are due in September.", url="https://x/fees", title="Fees")]
        fake_provider = MagicMock()
        fake_provider.generate.return_value = "Fees are due in September, per the context."

        with patch("app.agent.chat_engine.search", return_value=results):
            reply = get_reply(
                session=object(), corpus_config=CONFIG, query="when are fees due",
                llm_provider=fake_provider,
            )

        assert reply["text"] == "Fees are due in September, per the context."
        assert reply["sources"] == [
            {"url": "https://x/fees", "title": "Fees", "category": "General"}
        ]
        fake_provider.generate.assert_called_once()

    def test_falls_back_to_top_chunk_when_llm_raises(self):
        results = [_result(text="Fees are due in September for all students.")]
        fake_provider = MagicMock()
        fake_provider.generate.side_effect = RuntimeError("LLM down")

        with patch("app.agent.chat_engine.search", return_value=results):
            reply = get_reply(
                session=object(), corpus_config=CONFIG, query="q", llm_provider=fake_provider
            )

        assert "Fees are due in September" in reply["text"]
        assert reply["sources"]

    def test_passes_top_k_and_category_filter_through_to_search(self):
        with patch("app.agent.chat_engine.search", return_value=[]) as mock_search:
            get_reply(
                session=object(),
                corpus_config=CONFIG,
                query="q",
                top_k=3,
                category_filter="Housing",
            )
        _, kwargs = mock_search.call_args
        assert kwargs["top_k"] == 3
        assert kwargs["category_filter"] == "Housing"

    def test_uses_default_provider_when_none_passed(self):
        with (
            patch("app.agent.chat_engine.search", return_value=[]),
            patch("app.agent.chat_engine.get_llm_provider") as mock_factory,
        ):
            get_reply(session=object(), corpus_config=CONFIG, query="q")
        mock_factory.assert_called_once()

    def test_passes_history_through_to_the_prompt(self):
        results = [_result(text="Fees are due in September.")]
        fake_provider = MagicMock()
        fake_provider.generate.return_value = "answer"
        history = [HistoryTurn(role="user", text="earlier question")]

        with (
            patch("app.agent.chat_engine.search", return_value=results),
            patch("app.agent.chat_engine.build_prompt", return_value="prompt") as mock_build,
        ):
            get_reply(
                session=object(),
                corpus_config=CONFIG,
                query="q",
                llm_provider=fake_provider,
                history=history,
            )

        _, kwargs = mock_build.call_args
        assert kwargs["history"] == history

    def test_defaults_to_no_history_when_not_passed(self):
        results = [_result()]
        fake_provider = MagicMock()
        fake_provider.generate.return_value = "answer"

        with (
            patch("app.agent.chat_engine.search", return_value=results),
            patch("app.agent.chat_engine.build_prompt", return_value="prompt") as mock_build,
        ):
            get_reply(session=object(), corpus_config=CONFIG, query="q", llm_provider=fake_provider)

        _, kwargs = mock_build.call_args
        assert kwargs["history"] is None


class TestPrepareReplyContext:
    def test_shortcut_match_short_circuits_search(self):
        config = CorpusConfig(name="t", persona="p", navigation_shortcuts=[REGISTER_SHORTCUT])
        with patch("app.agent.chat_engine.search") as mock_search:
            shortcut, results = prepare_reply_context(
                session=object(), corpus_config=config, query="register for a course"
            )

        mock_search.assert_not_called()
        assert shortcut is REGISTER_SHORTCUT
        assert results == []

    def test_no_shortcut_returns_search_results(self):
        results_list = [_result()]
        with patch("app.agent.chat_engine.search", return_value=results_list) as mock_search:
            shortcut, results = prepare_reply_context(
                session=object(),
                corpus_config=CONFIG,
                query="q",
                top_k=3,
                category_filter="Housing",
            )

        assert shortcut is None
        assert results == results_list
        _, kwargs = mock_search.call_args
        assert kwargs["top_k"] == 3
        assert kwargs["category_filter"] == "Housing"


class TestStreamReplyEvents:
    def test_shortcut_yields_token_then_sources(self):
        events = list(
            stream_reply_events(
                query="register for a course",
                corpus_config=CONFIG,
                shortcut=REGISTER_SHORTCUT,
                results=[],
                llm_provider=MagicMock(),
            )
        )

        assert events[0]["type"] == "token"
        assert "Registering for a course" in events[0]["text"]
        assert events[1] == {
            "type": "sources",
            "sources": [
                {
                    "url": "https://example.com/register",
                    "title": "Registering for a course",
                    "category": "Navigation",
                }
            ],
        }

    def test_no_results_yields_no_results_message(self):
        events = list(
            stream_reply_events(
                query="q",
                corpus_config=CONFIG,
                shortcut=None,
                results=[],
                llm_provider=MagicMock(),
            )
        )

        assert events == [
            {"type": "token", "text": NO_RESULTS_MESSAGE},
            {"type": "sources", "sources": []},
        ]

    def test_streams_provider_chunks_then_sources(self):
        results = [_result(text="Fees are due in September.", url="https://x/fees", title="Fees")]
        fake_provider = MagicMock()
        fake_provider.stream.return_value = iter(["Fees ", "are due ", "in September."])

        events = list(
            stream_reply_events(
                query="when are fees due",
                corpus_config=CONFIG,
                shortcut=None,
                results=results,
                llm_provider=fake_provider,
            )
        )

        tokens = [e["text"] for e in events if e["type"] == "token"]
        assert tokens == ["Fees ", "are due ", "in September."]
        assert events[-1] == {
            "type": "sources",
            "sources": [{"url": "https://x/fees", "title": "Fees", "category": "General"}],
        }

    def test_passes_history_through_to_the_prompt(self):
        results = [_result(text="Fees are due in September.")]
        fake_provider = MagicMock()
        fake_provider.stream.return_value = iter(["ok"])
        history = [HistoryTurn(role="user", text="earlier question")]

        with patch("app.agent.chat_engine.build_prompt", return_value="prompt") as mock_build:
            list(
                stream_reply_events(
                    query="q",
                    corpus_config=CONFIG,
                    shortcut=None,
                    results=results,
                    llm_provider=fake_provider,
                    history=history,
                )
            )

        _, kwargs = mock_build.call_args
        assert kwargs["history"] == history

    def test_falls_back_to_top_chunk_when_stream_raises(self):
        results = [_result(text="Fees are due in September for all students.")]
        fake_provider = MagicMock()
        fake_provider.stream.side_effect = RuntimeError("LLM down")

        events = list(
            stream_reply_events(
                query="q",
                corpus_config=CONFIG,
                shortcut=None,
                results=results,
                llm_provider=fake_provider,
            )
        )

        tokens = [e["text"] for e in events if e["type"] == "token"]
        assert any("Fees are due in September" in t for t in tokens)
        assert events[-1]["type"] == "sources"
